...
int main() {
    ...
    FileStorage fs("data/MANO_RIGHT.yml", cv::FileStorage::READ);
    Mat J_regressor_prior, hands_components, f, J_regressor, kintree_table, J, weights_prior, hands_coeffs, weights, vert_sym_idxs, posedirs,
            hands_mean, v_template, shapedirs, trans, pose, betas, full_pose;

    Mat selected_components, pose_coeffs, full_hand_pose;
...
//    fs["vert_sym_idxs"] >> vert_sym_idxs;
    ...
    if (flat_hand_mean){
        hands_mean = Mat::zeros(hands_components.rows, 1, CV_64F);
    } else{
        fs["hands_mean"] >> hands_mean;
    }
...
//    std::cout << "hands_mean: " << hands_mean.size << " and channels: " << hands_mean.channels() << std::endl;
    ...
    hands_coeffs = hands_coeffs.colRange(0, ncomps);
...
//    std::cout << selected_components << std::endl;
    ...
    pose_coeffs = Mat::zeros(rot + selected_components.rows, 1, CV_64F);
    // update pose coeffs
    vconcat(Mat(rot, 1, CV_64F, corr_global_rot), Mat(ncomps, 1, CV_64F, next_ncomps_to_hand_pose), pose_coeffs);

    full_hand_pose = (pose_coeffs.rowRange(rot, rot + ncomps).t() * selected_components).t();
    vconcat(pose_coeffs.rowRange(0, rot), hands_mean + full_hand_pose, full_pose);
...
//    std::cout << "shapedirs: " << model.shapedirs.size << std::endl;
    ...
    std::ofstream meshfile ("MANO_Mesh.obj");
    if (meshfile.is_open())
    {
        ...
        std::cout << "Finish writing MANO Mesh!" << std::endl;
    }
